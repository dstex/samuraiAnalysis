...
def planContour(pltVar,pltVarLbl,crds):
    ...
    if crds == 'map':
        ...
        if pltFT:
            plotFLpath(flLon,flLat,crds,proj=proj,dubLine=True)

        if vecType == 'vec':
            plotVec(lon[1::5],lat[1::5],u[int(lev*2),1::5,1::5],
                    v[int(lev*2),1::5,1::5],'map',proj=proj,scale=200)
        if vecType == 'barb':
            plotBarbs(x1d[1::5],y1d[1::5],u[int(lev*2),1::5,1::5],
                      v[int(lev*2),1::5,1::5],'map',proj=proj)
        if vecType == 'stream':
            plotStream(x1d,y1d,u[int(lev*2),:,:],v[int(lev*2),:,:],'map',proj=proj,strmRel=strmRel)
        ...
        if plotXSloc:
            ...
            if noBorder:
                if strmRel:
                    saveStr = '{}NB_{}{}_{}_{}-SR_{:.1f}km_xsLoc.{}'.format(savePath,dtSave,runIdSv,'map',pltVarLbl,lev,fType)
                    fig.savefig(saveStr,bbox_inches='tight',pad_inches=0)
                if strmRel:
                    saveStr = '{}NB_{}{}_{}_{}_{:.1f}km_xsLoc.{}'.format(savePath,dtSave,runIdSv,'map',pltVarLbl,lev,fType)
                    fig.savefig(saveStr,bbox_inches='tight',pad_inches=0)
            else:
                if strmRel:
                    saveStr = '{}{}{}_{}_{}-SR_{:.1f}km_xsLoc.{}'.format(savePath,dtSave,runIdSv,'map',pltVarLbl,lev,fType)
                else:
                    saveStr = '{}{}{}_{}_{}_{:.1f}km_xsLoc.{}'.format(savePath,dtSave,runIdSv,'map',pltVarLbl,lev,fType)
    ...
    if crds == 'xy':
        ...
        if plotXSloc:
            ...
            if strmRel:
                ...
                saveStr = '{}{}{}_{}_{}_{:.1f}km_xsLoc.{}'.format(savePath,dtSave,runIdSv,'xy',pltVarLbl,lev,fType)
...
def xsContour(pltVar,pltVarLbl,xsStrtPt,xsEndPt,xsIX,crds):
    ...
    if crds == 'map':
        ...
        fig,ax = plotXS_mapCrds(pltVar,lon,lat,alt,u,v,w,xsStrtPt,xsEndPt,
                      ...
                      vecType=vecTypeXS,figsize=(15,5),xCrd=xCrd,strmRel=strmRel)
    ...
    if crds == 'xy':
        if pltFTxs:
            ...
            fig,ax = plotXS(pltVar,x1d,y1d,alt,u,v,w,xsStrtPt,xsEndPt,
                          ...
                          vecType=vecTypeXS,figsize=(15,5),
                          pltFTxs=pltFTxs,fl_x=fl_x,fl_y=fl_y,
                          strmRel=strmRel,scaleVmag=scaleVmag,
                          doWcont=doWcont,doUcont=doUcont)
        else:
            fig,ax = plotXS(pltVar,x1d,y1d,alt,u,v,w,xsStrtPt,xsEndPt,
                          pltFlag=pltVarLbl,dT=dT,runId=runId,xsIX=xsIX+1,
                          vecType=vecTypeXS,figsize=(15,5),
...
if (pltFT and plotPlanViews) or (pltFTxs and plotXScts):
